...
图片只在当前浏览器中处理，不会上传服务器。Netlify 会像普通静态网站一样收到页面请求
和 HTML、JavaScript、Web Worker、WASM 等静态资源请求，但不会收到用户选择的图片、
文件名或处理结果。网站不存储图片或路径，也不接入统计、远程字体或第三方运行时脚本。
关闭或刷新页面后，尚未下载的结果无法恢复。

## 三种处理模式

| 模式 | JPG / JPEG | PNG | 静态 WebP | 动态 WebP | BMP |
| --- | --- | --- | --- | --- | --- |
| 转为 JPG 并写入标签 | 保持 JPEG 图像数据，直接写入 XMP | 保持原尺寸转为 JPG | 保持原尺寸转为 JPG | 不支持转换 | 保持原尺寸转为 JPG |
| 保持原格式并写入标签 | 保持 JPG | 保持 PNG | 保持 WebP | 保持动态 WebP | 不支持 |
| 只检查标签 | 检查，不修改 | 检查，不修改 | 检查，不修改 | 检查，不修改 | 不支持 |

“转为 JPG”会保持原尺寸，把透明区域合成白色背景，并以质量 100、4:4:4 色度采样和嵌入
...
运行时环境变量。CSP 的 `connect-src 'self'` 仅允许 Worker 从同源加载构建后的
MozJPEG WASM；不会开放第三方连接。
